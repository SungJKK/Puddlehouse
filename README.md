...
- [x] **`cli_design.md` documents CSV/JSON write support that isn't implemented** — The `--file` flag is described as accepting "a local Parquet, CSV, or JSON file", but `cli/commands/data.py` rejects anything that isn't `.parquet` with an explicit error. The README Known Limitations section acknowledges this gap, but the CLI design doc does not.
- [x] **`puddlehouse_design.md` audit log table is missing the `VACUUM` operation** — The documented operations are `REGISTER`, `DEREGISTER`, `SNAPSHOT`, `DELETE`, `LINEAGE`, `VIEW`. But `catalog/manager.py` also writes a `VACUUM` audit entry on each file deletion during vacuum. This operation is never listed in the docs.
...
- [x] **`puddlehouse_design.md` describes delete-at-query-time as a "known gap" — it's implemented** — The design doc states the query engine "does not yet apply delete files at read time" and calls it a known gap. `query/engine.py` now applies equality deletes via DuckDB EXCEPT clauses. The doc was never updated after the fix was shipped.
- [x] **`api_design.md` shows `updated_at` in views responses but `_serialize_view` omits it** — Both list and get view responses in the design doc include `updated_at`. The `View` model has the field, but `api/routers/views.py` `_serialize_view` never includes it.
- [x] **`table_design.md` says "eleven tables" and omits `catalog_quality_contracts`** — The doc states "There are eleven tables" but twelve exist. `catalog_quality_contracts` has no section, is absent from the Relationships diagram, and has no entry in the Indexes table.
- [x] **`table_design.md` `catalog_snapshots.row_count` description didn't match implementation** — Originally described as "after logical deletes" when it wasn't; corrected to "not adjusted for logical deletes" after auditing. Then f48b2e8 made row_count actually delete-aware, making the description stale again. Final description now correctly reads: cumulative row count adjusted for logical deletes.
