...
- [ ] **`api_design.md` shows `updated_at` in views responses but `_serialize_view` omits it** — Both list and get view responses in the design doc include `updated_at`. The `View` model has the field, but `api/routers/views.py` `_serialize_view` never includes it.
